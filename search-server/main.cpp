...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus document_status) const {
        return FindTopDocuments(raw_query, [document_status](int document_id, DocumentStatus status, int rating) { document_id += rating; return status == document_status; });
    }
...
void RunTestImpl(const TestFunction& testFunction, const string& functionName) {
    testFunction();
...
void TestAddingDocumentContent() {
    SearchServer server;
    server.AddDocument(0, "test adding the document"s, DocumentStatus::ACTUAL, {1, 2, 3, 4, 5});
...
void TestExcludeMinusWordsFromAddedDocumentContent() {
    SearchServer server;
    server.AddDocument(0, "test exclude minus words"s, DocumentStatus::ACTUAL, {1, 2, 3});
    ASSERT(server.GetDocumentCount() == 1);
    const auto found_docs = server.FindTopDocuments("test"s);
    ASSERT_EQUAL(found_docs.size(), 1u);
...
void TestMatchingDocumentContent() {
    ...
    ASSERT_HINT(words.empty(), "Document with minus words must not be finding"s);
    }
}
...
void TestCalculateDocumentRelevance() {
    ...
    server.SetStopWords("и в на"s);
    server.AddDocument(0, "белый кот и модный ошейник"s,                DocumentStatus::ACTUAL, {8, -3});
    server.AddDocument(1, "ласковый кот пушистый кот пушистый хвост"s,  DocumentStatus::ACTUAL, {7, 2, 7});
    server.AddDocument(2, "ухоженный кот выразительные глаза"s,         DocumentStatus::IRRELEVANT, {5, -12, 2, 1});
    server.AddDocument(3, "ухоженный скворец евгений"s,                 DocumentStatus::BANNED, {9});
    const auto found_docs = server.FindTopDocuments("кот"s);
    ASSERT_EQUAL(found_docs.size(), 2u);
